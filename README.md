...
- [x] ## 有1、2、3、4个数字，能组成多少个互不相同且无重复数字的三位数？都是多少？
...
## 有五个学生，每个学生有3门课的成绩，计算出平均成绩，况原有的数据和计算出的平均分数存放在磁盘文件"stud"中
...
void printfPerson(struct info);

int main(void) {
    struct info stu = {.name="zhangsan", .num=23, .profession='t', .sc.course="math"};
    struct info stu1 = {.name="wangming", .num=223, .profession='s', .sc.score=23};
    printfPerson(stu);
    printfPerson(stu1);
    return 0;
}

void printfPerson(struct info persion) {
    printf("name:%s\n", persion.name);
    printf(" num:%d\n", persion.num);
    printf(" profession:%c\n", persion.profession);
    if (persion.profession == 't')
        printf(" course:%s\n", persion.sc.course);
    else if (persion.profession == 's')
        printf(" score:%d\n", persion.sc.score);
...
enum level getLevel(int);

int main(void) {
    enum level flag = getLevel(18);//flag 的类型为enum level
    printf("%d", flag);
    return 0;
}

enum level getLevel(int digit) {
    if (digit <= 60)
        return low;
    else if (digit >= 60 && digit <= 79)
        return middle;
    else
        return Hight;
}
...
enum level getLevel(int num);

enum level getAllValue(enum level level);

int main(void) {
    srand((unsigned) time(NULL));
    getAllValue(hight|middle);
    return 0;
}
...
enum level getAllValue(enum level level) {
    while (true) {
        int a = rand();
        enum level l = getLevel(a);
        if ((l != other) && ((l & level) == l)) {
            printf(" %d", a);
        }
    }
}
